...
> ノート（つぶやき）の英訳は当初この非ゴールに入れていたが、Phase 5 で方針を変えて対応した。理由は §6 を参照。
...
| 対象 | 方針 |
|---|---|
| `lib/projects.ts` (14箇所の日本語) | `description` / `learned` の型を `Record<Locale, string>` に変更（Phase 5 で実施）。データはデータのまま持ち、辞書には移さない（プロジェクト追加時に1ファイルで完結させるため） |
| `content/notes.json`（つぶやき） | **翻訳する**（Phase 5 で方針変更）。当初は「都度英訳する運用は続かない」として見送る想定だったが、実データは9件・全文752文字と記事1本の1/4以下で、量を理由に見送る根拠が無かった。`content: { ja, en }` 形式に変更 |
...
| 5 | `lib/projects.ts` と `content/notes.json` の多言語化 | 0.25日 | ✅ 完了 |
