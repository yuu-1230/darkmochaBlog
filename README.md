...
- **Theme Concept**: **VS Code IDE Style (Zen Mode)**
  - **Base Colors**:
...
- **SEO & AEO Strategy**:
  ...
  - **Mobile Responsiveness**: スマホ閲覧時はIDEレイアウトを簡略化し、コンテンツ閲覧性を優先（予定）。
...
│   │   ├── activity-bar.tsx # 左端のトグルボタン (不要なアイコンを排除しシンプル化)
...
│   │   └── title-bar.tsx    # 最上部のウィンドウ枠 (Zen Mode風シンプルデザイン)
...
- [ ] **モバイル対応 (レスポンシブデザイン)** 👈 Priority High
